...
export function calculateForecast({ game, ratings, controls }) {
  ...
  const marketImplied =
    adapterMarketProbability
    ?? removeVig(game.cowboysMoneyline, game.opponentMoneyline);
  ...
  if (marketImplied !== null) {
    const pairedSportsbookCount = Math.max(
      0,
      Math.floor(Number(game.sportsbookCount) || 0),
    );
    drivers.push({
      ...
      evidence: adapterMarketProbability === null
        ...
        : `Median of ${pairedSportsbookCount} independently vig-adjusted sportsbook probabilities.`,
    });
  }
