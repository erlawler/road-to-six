...
| GitHub | COMPLETE WITH SOCIAL PREVIEW UPLOAD OPEN | The public [erlawler/road-to-six](https://github.com/erlawler/road-to-six) repository is current and pinned on Eric Lawler's profile. The `v1.0.0` tag and GitHub release are published. The active `Protect main` ruleset requires an up-to-date pull request, `validate`, CodeQL, resolved review threads, squash merge, and linear history while blocking force pushes and deletion. Dependabot alerts, security-update proposals, secret scanning, and push protection are enabled. The custom social-preview upload awaits browser file-access permission. |
