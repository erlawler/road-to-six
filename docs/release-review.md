...
**Private hosted candidate:** CURRENT VERSION COMPLETE; OPTIMIZED REFRESH PENDING
...
The optimized workspace build and live odds integration are release ready for a private hosted refresh. Runtime AI reached OpenAI successfully but the provider returned `insufficient_quota`, so the live AI gate remains blocked until API billing or prepaid credit is enabled for the dedicated project. The current owner-only candidate remains available while the optimized build awaits redeployment and smoke testing. Public hosting remains outside the current approval.
...
| Private hosting | IN PROGRESS | The current owner-only [Road to Six release candidate](https://road-to-six-erl.erlrickylre.chatgpt.site) remains deployed with one user and no groups. Provider credentials are stored as hidden secrets. The July 27 optimized build passes locally but still requires private redeployment and an owner-authenticated smoke test. |
...
- Local and prior owner-authenticated hosted checks returned five current Dallas events from The Odds API without exposing the key
...
- The prior owner-authenticated hosted smoke test verified the page, budget endpoint, current odds response, deterministic forecast fallback, semantic control group, and lack of horizontal overflow
...
- [ ] Codex: deploy the July 27 optimized build to the owner-only site and run its private smoke test.
