...
The public GitHub repository and optimized owner-only hosted candidate are complete. Version 6 passed the local build, automated test, GitHub CI, and owner-authenticated private smoke gates. Live odds are enabled through a six-hour shared cache, current source evidence is visible, and the deterministic explanation remains available. Runtime AI is securely configured but remains blocked by OpenAI provider quota. Public hosting remains blocked until Eric explicitly approves the private candidate and the live Runtime AI gate is completed.
