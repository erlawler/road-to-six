...
The earlier public GitHub repository and final owner-only v1.0.0 preview are available for review. Live odds are enabled through a six-hour shared cache, selected-game applicability is visible, and the deterministic explanation remains available. After billing was enabled, the live Runtime AI baseline and four-scenario scorecard passed with no fallback. The v1.0.0 build, lint, 73-test suite, 12-case AI evaluation, dependency audit, and authenticated hosted smoke test pass. Eric Lawler approved the final GitHub push, version tag, GitHub release, public hosting, and signed-out production smoke test on July 29, 2026.
